...
- [The Game of Anvil](#the-game-of-anvil)
- [The Core Loop - How to Play](#the-core-loop---how-to-play)
- [Throwing Stones - How to Roll Dice](#throwing-stones---how-to-roll-dice)
- [Aftermath \& Beats](#aftermath--beats)
- [Combat, Damage \& Staying Alive](#combat-damage--staying-alive)
- [Alternatives To Combat - Social Interaction](#alternatives-to-combat---social-interaction)
- [Moving Through Environments](#moving-through-environments)
...
## PHASE 4 - EXTRACT AND DOWNTIME

**Debrief.** GM and players review the mission together.

- Did Heat hit any Hammer thresholds?
- Primary objective complete?
- Secondary objectives complete?
- Was ANVIL exposed? Were civilians or factions involved?
- What rumors spread?
...
## FUSE vs. BLAST - what does each die do?
...
- `T2` BLAST 2+ to handle, one Explosive hit destroys
- `T2[++]` BLAST 2+ to handle, two Explosive hits destroy
- `T[++]` cannot be handled by standard actions, two Explosive hits destroy
- `T4[++]` BLAST 4+ to handle, two Explosive hits destroy

## Smother

If a dwarf performs a **standard action** and assigns **two 1s**, reduce the Heat gauge by 3.
...
- **Clean hit to GRIT:** marks all GRIT.
...
- roll an extra d6 when throwing stones
- discard the lowest die before assigning FUSE and BLAST
...
- roll an extra d6 for each item
...
- Your main weapon and tool.
- Raises your base CMP die to d6.
...
- Contents depend on Squad Reputation and Ember investment during downtime. 
- The stash is not a shop, dwarves do not browse it. 
- ANVIL decides what is available and they requisition what they need.
...
1. The squad is assigned an objective by the Hand.
2. They're given what intel exists.
3. They drop in.
4. They complete objectives.
5. They extract.

Every mission has a map and a sheet.

- The sheet handles the macro structure. 
- The map handles the room level detail.
...
- Provided by the Hand at mission assignment. 
- Official, possibly outdated, and occasionally wrong.
- Gives:
  - Rough map layout
  - Primary objective
  - Kn own faction presence
  - Recommended drop points

**Scout Intel:**

- Firsthand reconnaissance gathered during downtime via the Scout action.
-  Is always reliable.
-  Can fill in:
   -  Patrol routes
   -  Enemy Temper values
...
- Rooms they have intel on are filled in. 
- Everything else is dark until they move through it. 
- Players can see their entry point and any directly adjacent rooms.
- Scout intel is represented as additional keyed rooms.
...
- Guarded entrances have patrols stationed
- Structural entrances require breaching
- Unique entrances require specific gear or skills to use
- Some entrances are a mix of all three
...
- **Primary objective:** 3 Embers per dwarf. 
  - Completing only primary should be able to cover basic downtime needs.
- **Each secondary objective:** 2 Embers per dwarf.
  - Two secondaries plus primary puts the squad in a comfortable position.
...
> **04. FORGE HALL**
> *A large hall filled with dark smoke and glowing furnaces.*
> 4x Wardens (T3, sword), 1x Smithy (T2, hammer)
> [SCALD]
...
**1. Extraction**
- Get a target out, dead or alive depends on the mission.
- Primary success is the target leaves with the squad.
- Complications: target is unwilling, target is fragile.

**2. Elimination**
- Remove a specific target permanently.
- Primary success is confirmed removal.
- Complications: target is protected, target is mobile.

**3. Sabotage**
- Destroy, disable or corrupt something.
- Primary success is the thing no longer functions.
- Complications: is guarded, destroying it has consequences, must look like an accident.

**4. Recovery**
- Retrieve something without destroying it.
- Primary success is the object leaves intact.
- Complications: object is large, fragile, cursed, contested by another faction.

**5. Surveillance**
- Observe without being observed.
- Primary success is leaving with the intel.
- Complications: target is unpredictable, observation window is limited, squad cannot engage.

**6. Interruption**
- Stop something from happening.
- Primary success is the event never occurs.
- Complications: squad does not know exactly when, multiple triggers exist, stopping it causes fallout.

**7. Escort**
- Get someone through hostile territory alive.
- Primary success is delivery.
- Complications: target is a liability, they generate Heat, they have their own agenda.

**8. Infiltration**
- Get in, complete a task, get out without raising any alarms.
- Primary success is zero trace.
- Heat management is the entire mission, success if Cold or Warm.
- The squad can still escape if Hot+ but the rewards will reflect that.
...
- Every mission needs a Shatterpoint Hammer.
- Hot and White Hot Hammers are optional but recommended for anything longer than six rooms.
- Hammers should radically change mission conditions.
- Write the specific hammer for your mission on the sheet.
- Define what changes, what closes, what activates, and what the squad has to deal with.

**Hammer checklist:**

- [ ] Does the Hammer change the physical space? (routes close, rooms flood, ceilings fall)
- [ ] Does the Hammer change faction behavior? (reinforcements, elite units, full alert)
- [ ] Does the Hammer affect at least one objective? (the target moves, the item is destroyed, the window closes)
- [ ] Is the Hammer something the players caused, not something that happens to them passively?
...
- Ember payout
- Squad Reputation
- Sets context of future missions
...
- All GRIT is fully recovered here.
- Dwarves who retire here roll a new operative.
- Any player who's dwarf died during the mission rolls a new operative here.
...
- When a dwarf gains a Scar roll d66
- A player may instead choose a scar.
- Write one sentence about how it happened and mark it on your Op sheet.
- If a dwarf rolls a Scar they already have it deepens. The GM and player decide together what that means.
- Each dwarf has 4 Scar slots.
- When all 4 are full and a new Scar would be gained, permanently lose 1 GRIT instead.
...
- Roll a d6.
- Add 1 extra d6 for every GRIT marked during the mission.
- If any die shows a 1 or 2, gain 1 Scar.
- Only ever gain 1 Scar per mission regardless of how many dice show 1-2.
- If playing a one-shot, roll a scar when the character is created.
...
Dwarven decompression. The dwarf spends time in the Overhold's lower halls, drinking, fighting, and relaxing.
...
- The GM and player define the item together within the prep stash guidelines.
- Higher cost for more powerful or unusual items.

### SCOUT (1-3 Ember)
*Reveal mission intel.*
The dwarf runs advance reconnaissance on a potential mission target.

- Reveals one piece of hidden mission intel per ember.
- Ex: layout, Temper of key enemies, Hammer thresholds, faction presence, or extraction complications.

### PULL STRINGS (3 Ember)
*Manipulate Glimmerhold politics.*
The dwarf works back channels inside Glimmerhold to shift faction relations, suppress rumors, or reposition ANVIL's political standing.

- May adjust Squad Reputation by 1 mark.
- May affect future mission assignments.

### INTERROGATE (1 Ember)
*Extract intel from prisoners or artifacts.*
The dwarf works on a mission asset:

- A captured enemy, a recovered document, a strange relic.
- Generates specific actionable intel or reveals lore about enemy factions.

### REINFORCE (2 Ember)
*Add temporary mission assets.*
The dwarf requisitions additional resources for the next mission:

- A support contact, a safe extraction point, a distraction, a piece of borrowed heavy equipment.
- These are single use mission assets.
...
- Reputation reflects how ANVIL's handlers, rivals and enemies perceive the squad.
- Reputation is not a measure of skill, it is a measure of tactics.
- A squad at Myth leaves absolutely nothing behind, no witnesses, no evidence, no name.
- A squad at Legend gets the job done but the job echoes out.
...
- Missions resolve without rumour, without evidence and without a name attached.
- The Overhold is unsure if they can be trusted fully.
- All Overhold Action costs +2 Embers.
...
- The Hand trusts them and unlocks better stash access.
- All Overhold Actions costs +1 Ember.
- There is low chatter in hostile networks.
- The squad is a pattern enemies have noticed but cannot explain.

**3 - Known**

The squad has a name, they may not have chosen it but their enemies know it.

- Base costs apply, standard stash access.
- Factions with good intel networks know ANVIL exists.
- They do not know the squad but they have started paying attention.

**4 - Feared**

The squad does not go unnoticed anymore.

- The Hand gives them priority
- All Overhold Action costs −1 Ember (minimum 1).
- The stash has better options, and the squad may call in one favour per campaign.
- Enemies have started preparing specifically for them.

**5 - Legend**

The squad's reputation arrives before they do.

- Full Overhold backing.
- All Overhold Action costs −2 Embers (minimum 1).
- The stash is deep, the Queen's Hand takes personal interest in their survival.
