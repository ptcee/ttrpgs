...
- [TOME 2: THE WORLD AT LARGE](#tome-2-the-world-at-large)
  ...
  - [SECTION 3: THE LANGUAGE OF ORRELIA](#section-3-the-language-of-orrelia)
  - [SECTION 4: THE LANDSCAPE](#section-4-the-landscape)
  - [SECTION 5: THE SEASONS AND THEIR FESTIVALS](#section-5-the-seasons-and-their-festivals)
...
The count starts at **20,** the die starts at **d6.** Represents the dungeon waking up to the party's presence.

Each Turn the party acts or makes noise: roll the Itch die and subtract the result from the total.

- **Itch hits 3:** Foreshadow danger.
- **Itch hits exactly 0:** Set to 3, foreshadow danger.
- **Itch drops below 0:** Trigger the dungeon's **Scratch**, then reset to 20.
- **Quick Rest:** step the Itch die up (d4>**d6**>d8>d10>d12>d20). Resets on leaving the dungeon for a Watch or more.

Itch advances on Turns only. Combat Rounds do not advance it. Cannot step above d20. The Itch die always **starts at d6**: a fresh dungeon is already alert.
...
**Light:** Torches generally have **(4\*)**. Determine who is carrying the lit torch and mark a use every 3 Turns. In darkness: all saves Hindered + gain Rattled on failed save. Lanterns provide steady light but need oil to restock.

**Locks:** MEAT save. Without appropriate tools: TN +2.

**Traps:** A trap the party finds and doesn't disarm is an alarm for both sides. If avoided or rearmed the party can use it deliberately. A trap reset after the party passes is a rear guard; anything following them triggers it first. A fresh trap (oil still wet, dust disturbed) that wasn't there last time means someone returned after the party's last visit.
...
A dungeon should have at least two factions within: groups with goals, fears, and a threshold for violence. Each faction has an **Alert State** that changes based on what the party does.

A faction doesn't need to be a group of humanoids. A nest of giant rats is a faction. 

**Alert States:** Unaware > Suspicious > Hostile > Hunting

|  | State | Notes |
|---|---|---|
| 0 | **Unaware** | Standard Reaction roll on first contact. |
| 1 | **Suspicious** | Reaction roll at −2. Scouts move one room toward the most recent noise per Turn. |
| 2 | **Hostile** | No Reaction roll - attacks on sight. A creature that escapes will escalate to Hunting. |
| 3 | **Hunting** | Each Turn the party spends in the dungeon, roll the Itch die. On a 1 or 2, a hunting group arrives at the party's location. |

**Escalating:**
- Fighting faction members: Unaware > Suspicious. Kill without survivors: stays Suspicious.
- Significant noise: one step up.
- Kill a faction leader: straight to Hunting.
- A Hunting faction that finds the party at rest gets a full round before the party can act.

**De-escalating:**
- Leave the dungeon: resets to Unaware over d4 sessions.
- Successful bribe or negotiation: one step down.
- Break the faction entirely: resets to Unaware, but there are downstream consequences, other factions notice, Standing may be affected.

A faction's **Alert State** should be noted on the GM's faction card and updated in play. See the Faction Card format in the GM reference materials.
...
A **Watch** is 6 hours. There are four per day: **Morning, Afternoon, Evening, Night.** The party typically travels during Morning and Afternoon, camps during Evening, and rests during Night, but pushing further always has a cost.

**Travel Sequence (each Watch):**
1. Pick direction.
2. Roll weather.
3. WITS save to navigate. Fail: drift into adjacent hex. Roll the Drift die for that hex before the encounter die (see Hex Cards in GM reference materials).
4. Roll the **Overland Itch** (see below).
...
The same logic as the dungeon Itch, scaled to travel. The world notices the party. The louder they are, metaphorically and literally, the faster complications happen.

**Starts at 20. Die starts at d6.** Roll the Overland Itch die after the party describes how they will travel. How many rolls depends on how loud their plan is. Feel free to also roll the Itch die if the party does something loud mid travel.

| Watch type | Itch rolls |
|---|---|
| **Quiet** - uneventful travel, no incidents | 3 rolls |
| **Active** - navigation save made, weather effect, minor incident | 4 rolls |
| **Loud** - combat, visible fire, loud mucksow, pushed through severe weather, evidence left behind | 5 rolls |

This should always be obvious from what happened, no tracking needed. When the players are describing how they travel, decide how loud they're being.

**Step the die UP when the party:**
- Fights a wandering encounter
- Camps with a visible fire in a dangerous hex
- Leaves evidence, a body, a trail, an uncleaned campsite
- Triggers a dungeon Itch Consequence and then leaves the hex
- Has a Loud Mucksow make noise
- Pushes through persistent severe weather instead of sheltering

**Step the die DOWN when the party:**
- Completes a full Watch without incident
- Takes a Long Rest inside proper shelter (building, cave)
- Deliberately changes route to lose a tail
- Cleans up evidence of passage (costs time, WITS save)
- Spends a Watch scouting a dungeon hex rather than moving

**The die does not step down on reset.** A party that burned the Itch to 0 at d12 resets the total to 20 but stays at d12. Earning the step-down requires careful travel.

**When the Overland Itch hits 0**, the Scratch triggers. Roll or pick based on what fits the current hex.

| d6 | Scratch Consequence |
|---|---|
| 1 | **Caught Up.** Something tracking them arrives. Roll Reaction. |
| 2 | **Roadblock.** The route is watched: guards, a faction patrol, a toll. Go around (+1 Watch) or talk through it. |
| 3 | **Word Travels Fast.** The next hex's factions starts at Suspicious before the party arrives. |
| 4 | **Followed In.** The next dungeon delve starts with the Itch at 15 instead of 20. |
| 5 | **Ambush.** A prepared encounter, creatures with chosen ground. First action is Bolstered. |
...
On a **1-3 result** on any seasonal table, roll a **d4**, that's how many Watches the weather lasts before rolling again. Skip Step 3 of the Travel Sequence for that many watches.

Each Watch of persistent severe weather the party chooses:

**Shelter:** No progress. 1 ration per goblin. Counts as a Quick Rest only. Does not step up the Overland Itch die.

**Push through:** Make progress. Apply the weather effect. Each goblin gains 1 STRESS and loses 1 COURAGE. Counts as a **Loud Watch** for Overland Itch (nothing is moving out there except you).
...
**Quick Rest (1 Turn in dungeon / part of a Watch overland):** clear d4+1 STRESS (d6+1 if eating). Any Suspicious or higher faction moves one space closer during a Quick Rest, the dungeon notices the party has gone quiet.

**Long Rest (1 Watch minimum):**
- Clear all STRESS. Clear conditions that allow it.
- Roll Itch die at the start and end of the rest.
- Any Suspicious or higher faction moves two spaces closer during a Long Rest.
- A Hunting faction during a Long Rest arrives. No roll needed.
- **Fire:** Full light restored. Steps up Dungeon Itch die for the duration, and steps up Overland Itch die once when the party leaves. All attempts to hide are Hindered.
- **Watch posted:** one goblin stays awake and gains no benefit from rest. On anything approaching: ZEST save 
    - success means the party gets a full turn to act before the threat arrives.
  
#### Scouting

Spending a Watch adjacent to a dungeon hex without entering generates **one Rumour** about it (roll on the regional table, or GM picks something specific). Two Watches of scouting gives two Rumours. Each scouting Watch costs 1 ration per goblin and rolls the encounter die normally.

A scouting Watch counts as a **Quiet Watch** for the Overland Itch and steps the die down, the party is careful, stationary and leaving no trail.

#### Hex Tracks

The world doesn't wait. Each hex with an active situation has a **track starting at 0.** Each session that passes without the party visiting, step it up by 1. At 4 the situation resolves.

| d6 | Resolution |
|---|---|
| 1 | **Resolved badly.** Evidence of violence. The dungeon, if any, is now empty. |
| 2 | **Dug in.** Whoever won has settled. Faction starts at Hostile on the party's next arrival. |
| 3 | **Abandoned.** Whatever was here left. Dungeon accessible but empty. |
| 4 | **Escalated.** Dungeon Itch starts at 15 on the next delve. |
| 5 | **Changed hands.** A third party arrived. Existing Rumours about this hex are now wrong. |
| 6 | **Reputation arrives.** NPCs have heard about the party. First Reaction at +2 or −2. |

The party can intervene in a track by visiting and resolving the situation themselves. A track at 3 is a time-pressure mission.
...
| d4 | Quirk | Effect |
|---|---|---|
| 1 | **Loud** | Combat or sudden noise triggers an immediate Itch roll *and* steps the die up *before* rolling. Overland any Watch is one step higher. Quiet > Active > Loud. |
| 2 | **Hungry** | Needs +1 ration/day or becomes Exhausted. |
| 3 | **Cowardly** | Combat or sudden noise makes it bolt. ZEST save or d4: 1: toward the entrance, 2: into the next space, 3: toward the nearest noise, 4: out of the dungeon. If it exits, the party retrieves it after the delve and rolls the Overland Itch die, it may have led something back. |
| 4 | **Curious** | If unattended, WITS save or it drops a random carried item. This counts as noise. Roll the Dungeon Itch die. The item is somewhere in the dungeon now. |
...
Players can rent the same mucksow over multiple delves. If it survives it bonds with the party.

**Bonded (survived 2+ delves):** the Mucksow gains a Loyalty stat like a Contact, starting at 2. Loyalty 3: it follows the party to the entrance rather than bolting. Loyalty 0: it bolts at the first opportunity. Losing a bonded Mucksow drops the handler contact's Loyalty by 1 immediately.
...
> THE CELLAR SAMPLE DUNGEON HERE
