...
1. [GM Quick Reference](#gm-quick-reference) - the page you open mid-session
2. [Running the Game](#running-the-game) - GM guidance and world design
3. [Core Mechanics](#core-mechanics) - stats, dice, stress, courage
4. [Character Creation](#character-creation) - steps, backgrounds, advancement
5. [Combat](#combat) - initiative, attacks, defence, vital blows
6. [Equipment](#equipment) - inventory, weapons, armour, scrolls
7. [Exploring Dungeons](#exploring-dungeons) - the Itch, procedures, factions
8. [Travel & Downtime](#travel--downtime) - travel, overland Itch, weather, jobs
...
**Dungeon Itch:** Starts at **20**, die starts at **d6**.
Roll the Itch die each Turn the party acts or makes noise. Subtract the result.
...
**Overland Itch:** Same logic. Starts at **20**, die starts at **d6**.
Roll the Itch die at the start of each Watch after the party describes what they are doing. Subtract the result.
...
Step die **UP:** fight a wandering encounter, visible fire in a dangerous hex, evidence left behind, dungeon Scratch triggered then left.
Step die **DOWN:** full Watch without incident, Long Rest in proper shelter, deliberate route change, cleaned evidence (WITS save), scouting Watch.
...
1. **Initiative** - Party has jump: go first. Uncertain: ZEST save. Surprised: enemies go first.
2. **Each round:** 1 action + 1 move (~20ft). Items marked **~** are free actions.
3. **Attack:** Roll weapon dice + any COURAGE dice (no Gob Die; no STRESS unless Hindered).
4. Assign one die as **Break** and one as **Damage**. COURAGE dice can go to either.
5. If Hindered: discard dice matching STRESS die (not crits).
6. Break die must beat target **DEF** to land.
7. Damage > **Shield** absorbs first (1 use per 1 damage); excess > STRESS.
8. STRESS full > **MEAT** takes damage. **3+ MEAT from one hit** > Vital Blow.
9. MEAT hits 0 > **death**.

**Critical hit:** Break and Damage dice show the same value. Roll an extra Damage die; add to total.
**Gang up:** Roll all attacks as one pool; assign one Break and one Damage from the whole pool.
**Morale Break:** Monster takes 3+ MEAT from one hit > ZEST save or flees/surrenders. Indomitable auto-succeeds.
...
3. If you are assigning a 6 for Geography, use Tundra for hexes above the centre of the map, Desert for all others. 
...
2. If any remaining COURAGE die or Gob Die meets/beats the TN: **success.**
...
6. **Take starting gear:** Rations, torch, rope, 1 weapon and 1 Hoard item. Pack size by background.
...
4. Assign one die from pool as **Break die** and one as **Damage die.**
5. If Hindered: discard dice matching STRESS die (except crits).
6. Break die must beat target DEF to hit.
7. Damage > Shield absorbs first (1 use per 1 damage); excess > STRESS.
8. STRESS full > MEAT takes damage. **3+ MEAT from one hit** > Vital Blow.
9. MEAT hits 0 > death.

**Hindered in combat:** Add 1 STRESS die to attack pool. Multiple hinders do not stack. Affects entire pool in Gang Up.
...
2. Roll **1d12 Flow**, **1d12 Form**, **1d12 Facet.** Describe spell.
3. Declare power: **1d6, 2d6, or 3d6.**
4. Roll all power dice simultaneously.
5. Two matching dice = Minor Mishap. Three matching = Major Mishap.
...
1. Pick direction.
2. Roll weather.
3. WITS save to navigate. Fail: drift into adjacent hex. Roll Drift die for that hex before encounter die.
4. Roll the **Overland Itch.**
...
4. Protection removed. The Underhold may shun the party. No jobs. 
5. If standings drops when at Bad Standing 4; Gobfather starts sending assassins.
